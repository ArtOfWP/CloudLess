...
class Query
{
     ...
     * @return Query
     */
    public static function createFrom($class, $lazy = false)
    {
        if (is_object($class)) {
            $class = get_class($class);
        }
        $main_table = strtolower($class);
        $query = new self($main_table);
    ...
    private static function setup_one_to_one_dependencies($lazy, $object, $query, $main_table)
    {
        ...
        foreach ($properties as $property) {
            ...
            if ($depends_on && $lazy) {
                ...
                $temp = new $depends_on();
                $gProperty = self::createFrom($temp)->where(Restriction::Eq($temp, 'Id'));
