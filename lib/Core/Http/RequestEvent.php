...
class RequestEvent
{
    ...
    public function loadFromPost($crudItem = null, $stripPrefix = '')
    {
        ...
        foreach ($uploads as $property => $upload) {
            ...
            } elseif (is_null($this->getPostRequest($property.'_hasimage')) && empty($values[$property])) {
                $values[$property] = '';
            } elseif (strpos($this->getPostRequest($property.'_hasimage'), 'ttp') == 1) {
            ...
            } elseif (isset($this->thumbnails[$property]) && $this->thumbnails[$property][0] == 'create') {
                $url = $this->getPostRequest($property.'_hasimage');
                $name = str_replace(' ', '-', urldecode(basename($url)));
                $name = str_replace('+', '-', $name);
                $path = UPLOADS_DIR.$folder.$name;
